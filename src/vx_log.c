...
static const char *level_tag(vx_log_level_t level) {
    switch (level) {
    case VX_LOG_ERROR: return "ERROR";
    case VX_LOG_WARN: return "WARN ";
    case VX_LOG_INFO: return "INFO ";
    case VX_LOG_DEBUG: return "DEBUG";
    case VX_LOG_TRACE: return "TRACE";
    default: return "?????";
    }
}

void vx_log_set_level(vx_log_level_t level) { g_level = level; }
vx_log_level_t vx_log_get_level(void) { return g_level; }

void vx_log_init_from_env(void) {
    const char *env = getenv("VX_LOG");
    if (env == NULL || *env == '\0') return;
    if (strcmp(env, "silent") == 0) g_level = VX_LOG_SILENT;
    else if (strcmp(env, "error") == 0) g_level = VX_LOG_ERROR;
    else if (strcmp(env, "warn") == 0) g_level = VX_LOG_WARN;
    else if (strcmp(env, "info") == 0) g_level = VX_LOG_INFO;
    else if (strcmp(env, "debug") == 0) g_level = VX_LOG_DEBUG;
    else if (strcmp(env, "trace") == 0) g_level = VX_LOG_TRACE;
}
...
void vx_logf(vx_log_level_t level, const char *file, int line, const char *fmt, ...) {
    ...
    char buf[1024];
    int n = snprintf(buf, sizeof(buf), "[vxworker %s %llu %s:%d] ", level_tag(level),
                     vx_now_us(), basename_of(file), line);
    if (n < 0) return;
